...
class HITLEvent(BaseModel):
    ...
    kind: int = Field(default=10020, description="Nostr kind (10020 = async INBOX_ITEM)")
    ...
    def from_request(cls, req: HITLRequest) -> "HITLEvent":
        ...
        return cls(
            ...
            # session_id / dw_pubkey / content are NOT NULL in the shared
            # llibreta table — always provide values.
            session_id=req.metadata.get("session_id") or f"hitl-{event_id}",
            dw_pubkey=req.metadata.get("dw_pubkey") or "hitl-hub",
